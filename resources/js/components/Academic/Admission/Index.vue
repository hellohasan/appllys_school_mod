<template>
	<custom-card :title="$t('Admission_list')" :url="'/admissions/add'" :text="$t('New_admission')" :icon="'fas fa-plus'">
		...
		<table class="table table-hover table-bordered" id="sampleTable">
			...
			<tbody>
				<tr v-for="list in lists" :key="list.id">
					...
					<td>{{ list.user.name }} <br>{{ list.user.phone }}</td>
					<td>
						{{ list.user.guardian.guardian.name }} <br>{{ list.user.guardian.guardian.phone }}
					</td>
					<td>
						{{ list.academic.level.details.details }}
					</td>
					<td>
						<span class='badge' :class="list.status ? 'badge-success' : 'badge-warning'">{{list.status ? 'Approved' : 'Pending'}}</span>
					</td>
					<td>
						<router-link :to="`/admissions/view/${list.custom}`" class="btn btn-success btn-sm" v-tooltip="'Show Admission'"><i class="far fa-eye"></i></router-link>
