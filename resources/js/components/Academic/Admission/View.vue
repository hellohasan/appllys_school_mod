<template>
	<custom-card :title="$t('Admission_details')" :url="'/admissions/add'" :text="$t('New_admission')" :icon="'fas fa-plus'">
		...
		<div id="printMe">
			...
			<template v-if="!loading">
					...
					<div class="col-md-9">
						<div class=form-row>
							<div class="form-group col-md-4 form-group-mb-0">
								<label for="name_bn">Session: {{session}}</label>
							</div>
							<div class="form-group col-md-4 form-group-mb-0">
				...
				<div class="form-row">
					<div class="form-group col-md-3 form-group-mb-0">
						<label for="data">Session: {{session}}</label>
					</div>
					<div class="form-group col-md-3 form-group-mb-0">
						<label for="data">Class: {{className}}</label>
					</div>
					<div class="form-group col-md-3 form-group-mb-0">
						<label for="data">
							<template v-if="groupDepartment">
								<template v-if="type == 1">Group:</template>
								<template v-if="type == 2">Department:</template>
								{{groupDepartment}}
							</template>
							<template v-else>
								Group: N/A
				...
				<div class="form-row">
					...
					<div class="form-group col-md-3 form-group-mb-0" v-for="(doc, i) in documents" :key="i">
				...
				<div class="form-group" style="margin-bottom: 55px;">
					<label class="form-check-label" for="exampleCheck1">আমি <span class="font-weight-bold text-italic">{{guardian.name}}</span> আমার ছেলে/মেয়ে/অনুগত <strong class="text-italic">{{name}}</strong> কে অদ্য {{session}} ইং শিক্ষাবর্ষের অত্র বিদ্যালয়ের ভর্তি পূর্বক অঙ্গীকার করিতেছি যে, বিদ্যালয়ের আরোপিত সকল শর্তাবলি মানিয়া চলিতে বাধ্য থাকিব। </label>
				</div>
		...
		methods: {
			...
			loadAdmissionDetails() {
				axios.get('/api/load-admission-details', { params: { custom: this.$route.params.custom } }).then(({ data }) => {
					...
					this.type = data.class_type
					this.session = data.session
					this.groupDepartment = data.groupDepartment
					this.year = data.year
					this.section = data.section

					this.custom = data.user.custom
					this.name = data.user.name
					this.phone = data.user.phone
					this.photo = data.user.photo
					this.created_at = data.user.created_at
					this.father = data.user.father[0]
					this.mother = data.user.mother[0]
					this.guardian = data.user.guardian.guardian_information
					this.information = data.user.information
					this.address = data.user.address
					this.academic = data.user.admission.academic
					this.previous_institute = data.user.previous_institute
					this.family_info = data.user.family_info
					this.documents = data.user.documents
					this.optional = data.user.admission.academic.optional_subject
					this.loading = false
				})
			}
		},
