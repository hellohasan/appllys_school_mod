        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title font-weight-bold">{{ $t('Group_section') }}</h3>
                ...
                <div class="card-body table-responsive p-0">
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        <tr v-for="(list, index ) in lists" :key="list.id">
                            ...
                            <td>
                                <button class="btn btn-primary btn-sm" @click="editSection(list)"><i class="far fa-edit"></i> {{$t('Edit')}}</button>
                            </td>
                        </tr>
                        </tbody>
                    </table>
        ...
        <div class="modal fade" id="myModal" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form @submit.prevent="editMode ? updateSection() : createSection()" @keydown="form.onKeydown($event)">
                        <div class="modal-body">
                            <form-group-input :form="form" v-model="form.name" name="name" :label="$t('Name')"></form-group-input>
                            <form-group-input :form="form" v-model="form.seat" type="number" name="seat" :label="$t('Seat')"></form-group-input>
...
export default {
    ...
    methods:{
        ...
        updateSection() {
            this.form.put('/api/academic-sections/' + this.editId)
                .then(() => {
                    this.loadAcademicSection();
                    $('#myModal').modal('hide');
                    Toast.fire({
        ...
        loadAcademicSection(){
            axios.get('/api/academic-sections').then(({data}) => this.lists = data)
        },
    },
    created(){
        this.loadAcademicSection();
