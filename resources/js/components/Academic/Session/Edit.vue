<template>
    <form @submit.prevent="updateAcademicSession" class="p-3">
        <div class="row">
            <div class="col-md-6">
...
export default {
    ...
    methods:{
        updateAcademicSession(){
            ...
            this.form.put('/api/academic-sessions/update/'+id)
                .then((response)=>{
                    Toast.fire({
                        icon: 'success',
    ...
    created() {
        axios.get(`/api/academic-sessions/edit/${this.$route.params.id}`)
            .then(({data})=>{
                this.form.fill(data)
                let list = [];
                $.each(data.academic_classes,function (key,value){
                    list.push({
                        id: value.id,
                        text: value.name,
                        admission_fee: value.pivot.admission_fee,
                    })
                })
