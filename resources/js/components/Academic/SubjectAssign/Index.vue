<template>
  <custom-card :title="$t('SubjectAssignList')" :url="'/subject-assign/new'" :icon="'fas fa-plus'" :text="$t('NewSubjectAssign')">
      ...
      <table id="myTable" class="table table-bordered table-striped display dt-responsive nowrap" style="width:100%">
        <thead>
          <tr>
            ...
            <th>{{ $t("Academic_Group") }}/{{ $t('Academic_Department') }}</th>
            <th>{{ $t("Section") }}/{{ $t('Department_year') }}</th>
          ...
          if (action == "delete") {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                axios.delete(`/api/delete-subject-assign/${id}`).then((res) => {
                  Toast.fire({
                    icon: "success",
            ...
            this.$router.push(`/subject-assign/${id}/show`);
          }
        }
        return;
      });
