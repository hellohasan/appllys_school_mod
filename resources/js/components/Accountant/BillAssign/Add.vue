        ...
        <form @submit.prevent="assignSubmit">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <custom-select @change="classChange" :options="classes" v-model="form.academic_class_id"
                                       :placeholder="$t('Select_One')"></custom-select>
                ...
                <div class="col-md-6">
                    <template v-if="this.form.academic_class_type == '1'">
                        <div class="form-group">
                            ...
                            <custom-select :options="groups" @change="changeGroup" v-model="form.academic_group_id"
                                           :placeholder="$t('Select_One')"></custom-select>
                    ...
                    <template v-if="this.form.academic_class_type == '2'">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <custom-select @change="changeDepartment" :options="departments"
                                               v-model="form.academic_department_id"
                                               :placeholder="$t('Select_One')"></custom-select>
            ...
            <template v-if="this.form.academic_class_id != ''">
                ...
                <div class="form-group">
                    ...
                    <custom-select :options="types" @change="changeBillType" v-model="form.bill_type"
                                   :placeholder="$t('Select_One')" name="bill_type"></custom-select>
                        ...
                        <div class="input-group">
                            <div class="input-group">
                                <input type="text" class="form-control input-sm" v-model="item.text"
                                       :readonly="item.id > 1" :required="item.id === 1"/>
                                <span class="input-group-btn" style="width:0px;"></span>
                                <input type="number" step="0.001" class="form-control input-sm" v-model="item.amount"/>
                                <div class="input-group-append">
                                    <button class="btn btn-outline-danger" type="button" @click="deleteItem(index)"><i
...
export default {
    name: "Add",
    components: {
        Select2, CustomSelect, CustomSelectEmpty, FormGroupSelect, CustomSelectMultiple
    },
    data() {
        return {
            form: new Form({
    ...
    methods: {
        changeBillType: function (e) {
            if (e === 'Specific') {
                ...
                if (classType === 0) {
                    axios.get('/api/load-student-for-class', {params: {class_id: this.form.academic_class_id}}).then((res) => {
                        this.students = res.data
                    })
                } else if (classType === 1 && this.form.academic_group_id !== '') {
                    axios.get('/api/load-student-for-class-group', {
                        params: {
                    ...
                    axios.get('/api/load-student-for-class-department-year', {
                        params: {
                            class_id: this.form.academic_class_id,
