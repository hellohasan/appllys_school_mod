      ...
      <table id="myTable" class="table table-bordered table-striped display dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <tr v-for="(item,index) in lists" :key="index">
            ...
            <td>
              <router-link :to="`/bill-assigns/${item.id}/view`" class="btn btn-success btn-sm"><i class="fas fa-eye"></i></router-link>
              <button class="btn btn-danger btn-sm" @click="deletePackage(item.id)"><i class="fas fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      deletePackage(custom) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axios
              .delete("/api/bill-assign-delete", { data: { custom: custom } })
              .then(() => {
