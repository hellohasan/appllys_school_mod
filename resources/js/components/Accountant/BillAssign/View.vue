      ...
      <tbody>
        ...
        <template v-if="bill.academic_class_type == 1">
          <tr>
            ...
            <td>{{bill.academic_group.name}}</td>
          </tr>
        </template>
        <template v-else-if="bill.academic_class_type == 2">
          <tr>
            <td class="text-right">{{$t('Academic_Department')}}</td>
            <td>{{bill.academic_department.name}}</td>
          </tr>
          <tr>
            <td class="text-right">{{$t('Department_year')}}</td>
            <td>{{bill.academic_year.name}}</td>
          </tr>
        </template>
        <tr>
          <td class="text-right">{{$t('SelectType')}}</td>
          <td v-if="bill.bill_type === 'All'">{{$t('AllStudent')}}</td>
          <td v-else>{{$t('SpecificStudent')}}</td>
        </tr>
        <tr>
          <td class="text-right">{{$t('TotalStudent')}}</td>
          <td>{{bill.bill_students_count | students}}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="2">
            <h4 class="text-center">{{ $t('BillItem') }}</h4>
...
<script>
  export default {
    ...
    methods: {
      loadBillStudent() {
        axios
          .get(`/api/bill-assign-view/${this.$route.params.custom}`)
          .then((res) => {
            this.bill = res.data;
