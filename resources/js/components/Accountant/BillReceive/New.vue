									...
									<div class="col-md-9 col-lg-9">
										<table class="table table-user-information table-striped table-bordered">
											<tbody>
												<tr>
													<td>{{ $t("AcademicID") }}</td>
													<td>{{student.academic_data.custom }}</td>
												</tr>
												<tr>
													<td>{{ $t('AcademicInfo') }}</td>
													<td>{{ student.academic_data.level.details.details }}</td>
												</tr>
												<tr>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="row">
							...
							<div class="col-6">
								<button :disabled="billForm.busy" type="button" @click.prevent="submitBillReceive" class="btn btn-primary"><i class="fas fa-save"></i> {{ $t("ReceiveNow") }}</button>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="row" v-if="student !== ''">
				<div class="col-6" v-if="printAble !== ''">
					...
					<div class="row mt-3">
						...
						<div class="col-12">
							<p>{{ $t('StudentName') }}: {{ student.student_data.name }} - ({{student.academic_data.custom }})</p>
						</div>
						<div class="col-12">
							<div class="d-flex justify-content-between">
								<div class="flex-fill">
...
<script>
	...
	export default {
		...
		methods: {
			...
			printReceipt(billId) {
				axios
					.get("/api/specific-bill-receipt", { params: { id: billId } })
					.then((res) => {
			...
			loadSpecificData(id) {
				axios
					.get("/api/receive-specific-student-bill", {
						params: { id: id },
					})
					.then((res) => {
			...
			classChange: function (e) {
				if (e) {
					...
					this.student = "";
					axios
						.get("/api/load-class-group-department", {
							params: { id: e },
						})
						.then((res) => {
			...
			changeGroup: function (e) {
				if (e) {
					this.student = "";
					axios
						.get("/api/load-only-group-section", { params: { id: e } })
						.then((res) => {
							this.sections = {};
