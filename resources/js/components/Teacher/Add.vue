<template>
  <form @submit.prevent="storeTeacher" class="p-3">
...
<script>
  ...
  export default {
    ...
    methods: {
      storeTeacher() {
        this.form.post("/api/teacher-store").then((res) => {
          Toast.fire({
            icon: "success",
      ...
      typeChange(e) {
        if (e === "1") {
          ...
          axios
            .get("/api/load-academic-group-all-list")
            .then(({ data }) => (this.lists = data));
        }
      },
      addNew() {
      ...
      loadDesignationList() {
        axios
          .get("/api/load-designation-list")
          .then(({ data }) => (this.designations = data));
      },
      loadSalaryScaleList() {
        axios
          .get("/api/load-salary-scale-list")
          .then(({ data }) => (this.scales = data));
      },
      loadReligiousList() {
        axios
          .get("/api/load-religion-list")
          .then(({ data }) => (this.religions = data));
      },
      loadBloodList() {
        axios
          .get("/api/load-blood-list")
          .then(({ data }) => (this.bloods = data));
      },
    },
    created() {
